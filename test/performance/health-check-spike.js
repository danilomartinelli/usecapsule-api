...
export default function () {
  ...
  if (!success) {
    ...
    console.warn(
      `Health check failed - Status: ${response.status}, Duration: ${duration}ms`,
    );
  }
  ...
  if (response.status === 200) {
    try {
      ...
      if (Math.random() < 0.05) {
        // 5% of requests
        const healthyCount = Object.values(healthData.services || {}).filter(
          (s) => s?.status === 'healthy',
        ).length;
        const totalCount = Object.keys(healthData.services || {}).length;

        console.log(
          `Spike test - System: ${healthData.status}, Services: ${healthyCount}/${totalCount} healthy, Duration: ${duration}ms, VUs: ${__VU}`,
        );
      }
    } catch (e) {
      healthCheckFailures.add(1);
...
export function setup() {
  ...
  console.log(
    'This test simulates sudden load spikes to verify system resilience',
  );
...
export function teardown(data) {
  ...
  console.log(
    'Check the results to verify the system handled load spikes gracefully',
  );
}
