 ...
 */
type TransactionHandler<T> = (connection: DatabaseTransactionConnection) => Promise<T>;
...
export class DatabaseService implements OnModuleDestroy {
  ...
  async maybeOneFirst<T>(query: QuerySqlToken<z.ZodType>): Promise<T | null> {
    return this.executePoolMethod(this.pool.maybeOneFirst.bind(this.pool), query);
  }
  ...
  async many<T>(query: QuerySqlToken<z.ZodType>): Promise<readonly T[]> {
    return this.executePoolMethod(this.pool.many.bind(this.pool), query);
  }
  ...
  async manyFirst<T>(query: QuerySqlToken<z.ZodType>): Promise<readonly T[]> {
    return this.executePoolMethod(this.pool.manyFirst.bind(this.pool), query);
  }
  ...
  async one<T>(query: QuerySqlToken<z.ZodType>): Promise<T> {
    return this.executePoolMethod(this.pool.one.bind(this.pool), query);
  }
  ...
  async maybeOne<T>(query: QuerySqlToken<z.ZodType>): Promise<T | null> {
    return this.executePoolMethod(this.pool.maybeOne.bind(this.pool), query);
  }
  ...
  async any<T>(query: QuerySqlToken<z.ZodType>): Promise<readonly T[]> {
    return this.executePoolMethod(this.pool.any.bind(this.pool), query);
  }
  ...
  async query<T extends z.ZodType>(
    ...
    return this.executePoolMethod(
      this.pool.query.bind(this.pool) as PoolQueryMethod<QueryResult<z.infer<T>>>,
      query,
    );
  }
