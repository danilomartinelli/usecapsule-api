 ...
 *
 * This module bootstraps the Billing Service microservice in the DDD architecture.
...
async function bootstrap(): Promise<void> {
  ...
  try {
    logger.log('Starting Billing Service microservice...');
    ...
    await app.listen();
    logger.log(
      `Billing Service microservice successfully started and listening on queue: ${rabbitConfig.queue}`,
    );
  } catch (error) {
    logger.error(
      'Failed to start Billing Service microservice:',
...
function setupGracefulShutdown(app: INestMicroservice, logger: Logger): void {
  const shutdown = async (signal: string) => {
    ...
    try {
      await app.close();
      logger.log('Billing Service microservice shut down gracefully');
      process.exit(0);
    } catch (error) {
      logger.error(
